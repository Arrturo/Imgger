...
function PostsAdminScreen() {
  ...
  return (
    <div>
      <h1 className="text-4xl text-center mb-5">Added posts in PySquad ({posts?.length})</h1>
        ...
        <Table striped hover responsive className="table-sm mb-5">
            ...
            <tbody className="text-center">
                {posts.map(post => (
                    <tr key={post?.node?.id} >
                        <td>{post?.node?.id}</td>
                        <td><Image className="max-h-28 " src={`http://127.0.0.1:8000/media/${post?.node.image?.file}`}></Image>
                        </td>
                        <td>{post?.node?.title}</td>
                        <td>{post?.node?.user?.username}</td>
                        <td>{(post?.node?.createTime)?.substring(0, 10)} {(post?.node?.createTime)?.substring(15, 19)}</td>
                        <td>
                            <Button varinat='light' className="btn-m"><i class="fa-regular fa-pen-to-square text-lime-500"></i></Button>
                            <Button varinat='danger' className="btn-m" ><i class="fa-solid fa-trash-can text-red-500"></i></Button>
                        </td>
                    </tr>
                ))}
            </tbody>
