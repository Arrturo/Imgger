...
import {getUserDetails, updateUserProfileByAdmin} from '../actions/userActions'
...
function UserEditScreen() {
    ...
    const submitHandler = (e) => {
      e.preventDefault()
      dispatch(updateUserProfileByAdmin({
        'id': userId,
        'username': username,
        'email': email,
        'isStaff': isStaff
  ...
  return (
    <div>
        <Link to='/admin/userlist' > <i class="fa-solid fa-caret-left"></i> Back</Link>
        <FormContainer className="formcontainer">
            <h1 className="text-5xl text-center mb-5">Edit user</h1>
                ...
                <Form.Group controlId='isstaff' className="mt-3">
                    <Form.Check type='checkbox' label='Is Staff' checked={isStaff} onChange={(e) => setIsStaff(e.target.checked)}>
