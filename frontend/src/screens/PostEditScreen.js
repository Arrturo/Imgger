...
function PostEditScreen() {
    ...
    const submitHandler = (e) => {
        e.preventDefault()
        dispatch(editPost({postId: id, title: title, description: description, category: category }));
        setTimeout(() => {
            navigate(`/post/${id}`)
        }, 1000)
    }
        ...
        <div>
            ...
            <Row className="px-5 py-5">
                ...
                <Form onSubmit={submitHandler}>
                    ...
                    <Form.Label className="text-xl">
                        <p className="text-center mb-3">Your current category: <CategoryItem name={post?.category?.name} /></p>
                        <i class="fa-regular fa-hand-point-right"></i> If you want to change the
                        category, choose one from list below
                        <span className="text-red-800">*</span>{" "}
                    </Form.Label>
                        <Form.Control
                            as="select"
                    ...
                    disabled={!userInfo.user.isStaff && post?.user?.id != userInfo?.user?.id}
                    >
