...
class PostTypeTestCase(TestCase):
    def setUp(self):
        ...
        self.post1 = Post.objects.create(title="Test Post1", category_id=self.category.id, image_id=self.image.id, user_id=self.user.id, is_private=False)
        self.post2 = Post.objects.create(title="Test Post2", category_id=self.category.id, image_id=self.image.id, user_id=self.user.id, is_private=False)
        self.post3 = Post.objects.create(title="Test Post3", category_id=self.category.id, image_id=self.image.id, user_id=self.user.id, is_private=False)
...
class CategoryTypeTestCase(TestCase):
    def setUp(self):
        ...
        self.image = Image.objects.create(name="TestImage", url="sum.jpg")
        Post.objects.create(title="Post 1", image_id=self.image.id, category=self.category, is_private=False)
        Post.objects.create(title="Post 2", image_id=self.image.id, category=self.category, is_private=True)
        Post.objects.create(title="Post 3", image_id=self.image.id, category=self.category, is_private=False)
